  ...
  return (
    <header className={`sticky top-0 z-40 navbar-transition w-full ${scrolled ? 'pt-4 px-4' : 'pt-0 px-0'}`}>
      <nav className={`mx-auto navbar-transition ${
        scrolled 
        ? 'max-w-5xl rounded-full bg-white dark:bg-[#1a1b26] border border-slate-200/80 dark:border-slate-800 shadow-[0_8px_32px_rgba(31,38,135,0.07)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.5)]'
        : 'max-w-none w-full rounded-none bg-white dark:bg-[#0f1016] border-b border-slate-200/50 dark:border-purple-900/30'
      }`}>
            ...
            <div className="hidden md:flex flex-1 max-w-sm lg:max-w-md mx-6 relative">
              ...
              <div className="relative w-full">
                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                  <Search className={`h-4 w-4 transition-colors ${isSearchFocused ? 'text-primary' : 'text-slate-400'}`} />
                </div>
                <form onSubmit={handleSearch}>
                  <input
                    id="search"
                    name="search"
                    className="block w-full pl-9 pr-3 py-1.5 border border-slate-200 dark:border-slate-700 rounded-full leading-5 bg-slate-100/90 dark:bg-slate-800/60 backdrop-blur-md placeholder-slate-500 dark:placeholder-slate-400 focus:outline-none focus:bg-white dark:focus:bg-[#1a1b26] focus:ring-2 focus:ring-primary/50 text-slate-900 dark:text-slate-100 sm:text-sm transition-all"
            ...
            <div className="flex items-center gap-3">
              {currentUser ? (
                <>
                  ...
                  {/* Profile Link (Avatar + RoleBadge) */}
                  <Link to="/profile" className="flex items-center group flex-shrink-0">
                    <img 
                      src={currentUser.avatar} 
                      alt="avatar" 
                      className="h-9 w-9 rounded-full object-cover border border-slate-200 group-hover:border-primary transition-colors"
                    />
                ...
                <Link 
                  to="/login" 
                  className="flex items-center px-4 py-2 border border-transparent text-sm font-bold rounded-full text-white bg-gradient-to-r from-primary to-purple-600 hover:from-purple-600 hover:to-primary shadow-lg shadow-primary/30 whitespace-nowrap transition-all transform hover:scale-105"
                >
              ...
              <div className="relative" ref={burgerMenuRef}>
                <button 
                  className="p-2 text-slate-500 dark:text-slate-400 hover:text-primary transition-colors hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full flex items-center justify-center relative"
                ...
                {/* Dropdown Menu */}
                {isMobileMenuOpen && (
                  <div className="absolute top-[120%] right-0 w-72 sm:w-80 bg-white/95 dark:bg-[#1a1b26]/95 backdrop-blur-2xl rounded-2xl shadow-[0_10px_40px_-10px_rgba(124,58,237,0.2)] dark:shadow-[0_10px_40px_-10px_rgba(0,0,0,0.5)] border border-slate-100 dark:border-slate-800 p-4 space-y-4 z-50 animate-scaleIn origin-top-right">
                      
                      {/* Search Bar inside Burger Dropdown */}
                      <div className="relative">
                          ...
                          <input
                              ...
                              value={searchQuery}
                              onChange={(e) => setSearchQuery(e.target.value)}
                              className="block w-full pl-9 pr-3 py-2 text-sm bg-slate-100 dark:bg-slate-800/80 rounded-xl border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-primary/50"
                              onKeyDown={(e) => {
                          ...
                          {/* Live search results inside dropdown */}
                          {searchQuery.trim().length >= 2 && (
                              <div className="mt-2 max-h-48 overflow-y-auto space-y-1 divide-y divide-slate-100 dark:divide-slate-800 bg-slate-50 dark:bg-slate-900/50 rounded-xl p-2 min-w-0">
                                  {isSearching ? (
                                      <div className="p-2 text-center text-xs text-slate-400 flex items-center justify-center">
                                          <Loader2 className="animate-spin w-3.5 h-3.5 mr-1" /> Tìm kiếm...
                                      </div>
                                  ) : searchResults.length > 0 ? (
                                      <>
                                          {searchResults.map(result => (
                                              <Link 
                                                  ...
                                                  className="flex items-center gap-2 py-1.5 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-lg px-1 transition-colors min-w-0"
                                              >
                                                  <img src={result.coverUrl} className="w-8 h-11 object-cover rounded shrink-0" alt=""/>
                                                  <div className="overflow-hidden min-w-0 flex-1">
                                                      <div className="text-xs font-bold text-slate-800 dark:text-slate-200 truncate">{result.title}</div>
                                                      <div className="text-[10px] text-slate-500 dark:text-slate-400 truncate">{result.author}</div>
                                                  </div>
                                              </Link>
                                          ))}
                                          <Link 
                                              to={`/search?q=${encodeURIComponent(searchQuery.trim())}`}
                                              onClick={() => setIsMobileMenuOpen(false)}
                                              className="block text-center pt-2 text-[10px] font-bold text-primary hover:underline"
                                          >
                                              Xem tất cả kết quả
                                          </Link>
                                      </>
                      ...
                      <div className="space-y-1.5 pt-1">
                          <Link 
                              ...
                              onClick={() => setIsMobileMenuOpen(false)} 
                              className={`block px-4 py-2.5 rounded-xl text-sm font-bold transition-colors ${isActive('/category/translated') ? 'bg-primary/10 text-primary' : 'text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}
                          >
                          ...
                          <Link 
                              ...
                              onClick={() => setIsMobileMenuOpen(false)} 
                              className={`block px-4 py-2.5 rounded-xl text-sm font-bold transition-colors ${isActive('/category/original') ? 'bg-primary/10 text-primary' : 'text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}
                          >
                              {NovelType.ORIGINAL}
                          </Link>
                          <Link 
                              to="/forum" 
                              onClick={() => setIsMobileMenuOpen(false)} 
                              className={`block px-4 py-2.5 rounded-xl text-sm font-bold transition-colors ${isActive('/forum') || location.pathname.startsWith('/forum') ? 'bg-primary/10 text-primary' : 'text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800/50'}`}
                          >
                              Diễn đàn
                          </Link>
                      </div>
                      ...
                      {/* User Context & Logout */}
                      {currentUser && (
                          <div className="border-t border-slate-100 dark:border-slate-800 pt-3 mt-1 flex flex-col gap-2">
                              <div className="px-3 py-1.5 bg-slate-50 dark:bg-slate-900/50 rounded-xl flex items-center gap-2">
                                  <UserIcon className="w-4 h-4 text-slate-400 shrink-0"/>
                                  <span className="text-xs font-bold text-slate-600 dark:text-slate-350 truncate">{currentUser.username}</span>
                              </div>
                              <button 
                                  onClick={() => {
                                      handleLogout();
                                  ...
                                  className="w-full text-left px-4 py-2.5 rounded-xl hover:bg-red-50 dark:hover:bg-red-900/20 font-bold text-red-600 dark:text-red-400 flex items-center transition-colors text-sm"
                              >
